...
class TraCIVehicle(KernelVehicle):
    ...
    def get_lane_leaders_speed(self, veh_id, error=-1001):
        """See parent class."""
        lane_leaders = self.get_lane_leaders(veh_id)
        return [error if lane_leader == '' else self.get_speed(lane_leader)
                for lane_leader in lane_leaders]

    def get_lane_followers_speed(self, veh_id, error=-1001):
        """See parent class."""
        lane_followers = self.get_lane_followers(veh_id)
        return [error if lane_follower == '' else self.get_speed(lane_follower)
    ...
    def add(self, veh_id, type_id, edge, pos, lane, speed):
        ...
        if veh_id in self.master_kernel.network.rts:
            if veh_id.split('_')[0] == "Agent" and veh_id not in self.__rl_ids:
                self.__rl_ids.append(veh_id)
