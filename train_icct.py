...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_name', help='environment name', type=str, default='palo_alto_hw')
    ...
    log_dir = os.path.join("./log/icct/", time.strftime('%Y-%m-%d_%H-%M-%S'))
    ...
    monitor_file_path = log_dir + method
    env = Monitor(env, monitor_file_path)
    eval_monitor_file_path = log_dir + 'eval_' + method
    ...
    callback = EpCheckPointCallback(eval_env=eval_env, best_model_save_path=log_dir,
                                    n_eval_episodes=args.n_eval_episodes, eval_freq=args.eval_freq,
                                    minimum_reward=args.min_reward)
