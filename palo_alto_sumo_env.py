        ...
        for _ in range(self.env_params.sims_per_step):
            ...
            if rl_agents_ids:
                ...
                if lane_follower:
                    ...
                    if lane_follower_acc < self.HARDBRAKE:
                ...
                if lane_leaders_ave_speed - self.k.vehicle.get_speed(rl_id) > self.SPEEDGAP:
                    print("Agent too slow. avg speed: {0:6.2f}, agent speed: {1:6.2f}".format(
                        lane_leaders_ave_speed, self.k.vehicle.get_speed(rl_id)
                    ))

                crash = (self.k.vehicle.get_crash(rl_id)) or \
                        (lane_follower_acc < self.HARDBRAKE) or \
                        (lane_leaders_ave_speed - self.k.vehicle.get_speed(rl_id) > self.SPEEDGAP)  # 35.791 mph
            else:
                crash = False
